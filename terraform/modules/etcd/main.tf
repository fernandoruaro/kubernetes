resource "aws_instance" "etcd" {
    ...
    subnet_id = "${element(var.subnet_ids, count.index % var.azs.size)}"
    associate_public_ip_address = true
    availability_zone = "${element(var.azs, count.index % var.azs.size)}"
...
resource "aws_alb_target_group_attachment" "etcd_client" {
  ...
  target_id = "${element(aws_instance.etcd.*.id, count.index % var.azs.size)}"
  port = 2379
}
